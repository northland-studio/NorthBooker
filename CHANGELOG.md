...
## v2.6.6 (Electron) — 2026-08-06

### 新增

- **功能**：使用说明页面 — 新增 `/guide` 页面，左侧可展开 / 折叠的三级目录（章节 + 小节），完整覆盖本项目全部功能的使用说明：快速开始、文件托管、在线文档、分享、账号邮箱、管理后台、个性化、桌面版、快捷键与常见问题
- **入口**：导航栏「用户协议」右侧新增「使用说明」链接，页面与入口均对 Electron 桌面版隐藏（仅网页版可见）

## v2.6.5 (Electron) — 2026-08-05

### 新增

- **功能**：多文件上传 — 上传对话框支持一次选择 / 拖拽多个文件批量上传，显示文件列表与总进度，单个文件可单独移除
- **功能**：分享落地页 — 新增路由 `/share/:token`，生成的分享链接可直接打开并预览文档内容（托管文档在线预览、在线文档渲染正文），支持访问密码验证
- **功能**：文档右键菜单新增「更名」选项，无需进入管理后台即可重命名文件

### 修复

- **修复**：文件夹返回位置 — 进入文件夹打开文档后返回时保持所在文件夹，不再跳回全部文件主页面
- **修复**：复制链接域名 — Electron 应用版复制链接 / 在线文档链接使用线上域名 `northbooker.xuanjian.top`，不再出现 `127.0.0.1` 本地地址
- **修复**：分享链接无法打开 — 此前分享链接无前端落地路由，访问 `/share/xxx` 直接失败；现已可通过分享页正常访问
- **修复**：在线文档分享入口统一 — 详情页分享与右键菜单分享统一为生成分享链接，复制链接统一复制线上文档地址
