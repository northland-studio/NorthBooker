...
- 长文朗读：四池并行流水线合成（上限 600 段），可拖动悬浮球实时展示各池状态
- 编辑器底部栏：版本历史 / 朗读 / 搜索 / 分享 / 评论等按钮置于底部固定栏（返回与目录在顶部），富文本工具栏独立在顶部
- 版本历史对比：对比相对上一版本的修改，新增（绿）/ 修改（黄）直接在正文文本中高亮（类朗读高亮），底部栏中间显示版本信息（编辑字数 / 版本时间 / 退出对比）
- TTS 进度悬浮球：合成进度以可拖动悬浮球展示，拖动定位、点击展开详情
- 文档权限：在线文档仅作者本人可编辑，管理员不可修改他人文档
...
- 邮箱绑定：发送验证邮件一键绑定，未绑定自动弹窗提示（2.6.1）
- 协作权限管理：文档可设「任何人可编辑 / 仅作者可编辑」，协作 WebSocket 与保存均鉴权（2.6.1）
- 协作控制面板：`/pages/:id/cowork_set` 集中管理协作权限、可见性与订阅（2.6.2）
- 个人主页：`/profile/:user` 展示贡献总字数、创作文档、上传文件、邮箱绑定与订阅情况（2.6.3）
- 上传者标注：文件托管系统展示上传者信息，所有作者/上传者信息均可点击进入个人主页（2.6.4）
- 多文件上传：一次选择 / 拖拽多个文件批量上传，显示列表与总进度（2.6.5）
- 分享链接：`/share/:token` 落地页直接预览内容，支持访问密码（2.6.5）
- 使用说明：`/guide` 页面左侧可折叠三级目录，覆盖全部功能说明（2.6.6）
- 响应式布局，适配桌面与移动端
